  ...
  return (
    <PageChrome>
      ...
      <section className="px-5 pb-24 sm:px-8 sm:pb-32">
        <div className="mx-auto max-w-6xl">
          <a href={localizedPath(locale, `/blogs/infos/${featured.slug}`)} className="group grid gap-8 rounded-3xl bg-stone p-4 sm:p-6 lg:grid-cols-[1.2fr_.8fr] lg:items-center">
            ...
            <div className="reveal p-3 sm:p-6">
              <p className="meta text-ink/40" data-blur>{es ? "Para leer" : "À lire"} · {category(featured.category)}</p>
          ...
          <div className="mt-16 grid gap-x-7 gap-y-14 sm:grid-cols-2 lg:grid-cols-3">
            {rest.map((article) => (
              <article key={article.slug} className="reveal group">
                <a href={localizedPath(locale, `/blogs/infos/${article.slug}`)}>
                  ...
                  <p className="meta mt-5 text-ink/35">{category(article.category)} · {article.date}</p>
