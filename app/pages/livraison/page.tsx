...
export default async function DeliveryPage() {
  ...
  return <PageChrome>
    <section className="px-5 pb-20 pt-16 sm:px-8 sm:pb-28 sm:pt-24"><div className="mx-auto max-w-6xl"><p className="meta text-ink/40" data-blur>{es ? "Entrega en 2–4 días" : "Livraison en 2 à 4 jours"}</p><Words as="h1" className="display mt-7 max-w-5xl text-[clamp(2.8rem,7vw,6rem)]" text={es ? "Tu Platypool llega / como más te *convenga*" : "Votre Platypool arrive / comme cela vous *arrange*"} /></div></section>
    ...
    <section className="px-5 py-20 sm:px-8 sm:py-28"><div className="mx-auto max-w-4xl text-center"><p className="meta text-ink/40" data-blur>{es ? "¿Tienes alguna pregunta sobre tu paquete?" : "Une question sur votre colis ?"}</p><Words className="display mt-6 text-[clamp(2rem,4.5vw,3.4rem)]" text={es ? "Estamos a un / correo de *distancia*" : "Nous sommes à un / e-mail de *distance*"} /><a href={localizedPath(locale, "/pages/contact")} className="pill reveal mt-9 bg-aqua text-ink hover:bg-ink hover:text-paper">{es ? "Contactar con Platypool" : "Contacter Platypool"}</a></div></section>
  </PageChrome>;
}
